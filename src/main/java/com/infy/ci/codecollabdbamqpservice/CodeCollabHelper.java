...
public class CodeCollabHelper {
	 static Logger log = Logger.getLogger(CodeCollabHelper.class.getName());
	
	static private CodeCollabHelper cihelper;
	private final static Object cihelperLock = new Object();
	private CodeCollabHelper(){
		
		
	}
	
	public static CodeCollabHelper getInstance(){
		synchronized (cihelperLock) {
			if(null == cihelper){
				cihelper = new CodeCollabHelper();
			}
		}
		return cihelper;
	}
